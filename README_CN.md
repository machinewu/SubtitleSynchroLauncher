...
| checkbox_shift_time_skip_comment   | 是否不对源字幕"Comment:"开头的注释行调轴（此作为界面勾选框的默认值）（此项只用在工序`fix_subtitle`中使用，且只对.ass字幕有效）。`yes`或`no`。                                                                                                                                                                             | `yes`                                                                                                              |
| checkbox_shift_time_skip_zero_time | 是否不对源字幕开始时间和结束时间都是0的行调轴（此作为界面勾选框的默认值）（此项只用在工序`fix_subtitle`中使用，对.ass字幕的注释行也有效）。`yes`或`no`。                                                                                                                                                                    | `yes`                                                                                                              |
| checkbox_shift_time_fix_negative   | 是否将调轴后的字幕中的负数时间置为0 （此作为界面勾选框的默认值）（此项只用在工序`fix_subtitle`中使用，对.ass字幕的注释行也有效）。`yes`或`no`。                                                                                                                                                                            | `yes`                                                                                                              |
| checkbox_clean_fanhuaji            | 是否将繁化姬信息的行清理（此作为界面勾选框的默认值）（此项只用在工序`fix_subtitle`中使用）。`yes`或`no`。                                                                                                                                                                                                              | `yes`                                                                                                              |
...
**注意**：工序`shift_source_subtitle_timeline_delay`、`shift_destination_subtitle_timeline_delay`和`fix_subtitle`会将字幕文件转为utf-8字符串输出，可视为自带了`convert_file_to_utf8`的功能。
...
| fix_subtitle                              | 修正字幕。目前支持修正源字幕.ass字幕"Comment:"开头的注释行还原成原本时轴、源字幕开始时间和结束时间都是0的行还原成原本时轴、调轴后的负数时间置为0、清理繁化姬信息的行这几个功能。                                                                                 | 两个参数：`源字幕文件（可任意编码，不一定utf-8）`, `调轴后字幕文件（可任意编码，不一定utf-8）`              | 修正后的字幕文件的utf-8字符串 |
